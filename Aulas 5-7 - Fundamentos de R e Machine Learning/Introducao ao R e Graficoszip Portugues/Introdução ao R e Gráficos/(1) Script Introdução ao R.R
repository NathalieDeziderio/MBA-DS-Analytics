...
# Também é possével digitar os comandos diretamente no console
...
# Após rodar este comando, as funçõees do tidyverse já podem ser utilizadas
...
# Neste momento, é importante entendermos o objetivo das funçõees
# As funçõees indicam a ação que deve executada pelo R
# No caso acima: pedimos que o R combinasse - c() - os números em um vetor
# Portanto, as funçõees são algoritmos que executam uma ação pré-estabelecida
# Um aspecto comum é que muitas funçõees demandam mais de um argumento
...
# Outra é não utilizar nomes muito parecidos com nomes de funçõees já existentes

# Seguindo com as funçõees, poderíamos utilizar o comando print()
...
sequencia <- c(1:10) # esta é uma forma de gerar uma sequéncia de números

# Se atribuir o mesmo nome a outro objeto, o objeto antigo é substituído

sequencia <- c(500:600)

# Existem vetores que guardam informaçõees de variadas classes:
...
# É possível realizar operaçõees com os vetores. A seguir, alguns exemplos:
...
# Fatores: contém variáveis categóricas com os possíveis valores estipulados
...
# Note que a sequéncia de levels e labels é a correta, isto é, mantém a ordem

# Outro tipo de objeto bastante relevante na análise de dados são os data frames

# Data frames: são os objetos que guardam informaçõees como nas bases de dados
# Assim, nos data frames estão colunas (variáveis) e linhas (observaçõees)
...
load("C:\\Users\\natha\\Downloads\\Introducao ao R e Graficoszip Portugues\\Introdução ao R e Gráficos\\(2) desempenho_aluno_escola.RData")

# Também podemos importar um arquivo em Excel (utilizando o readxl - tidyverse)
# A seguir, temos dados com a cotação de fechamento mensal de 4 açõees

library(readxl)
preco <- read_excel("C:\\Users\\natha\\Downloads\\Introducao ao R e Graficoszip Portugues\\Introdução ao R e Gráficos\\(2) precos_acao.xlsx")
...
pib_paises <- read.csv("C:\\Users\\natha\\Downloads\\Introducao ao R e Graficoszip Portugues\\Introdução ao R e Gráficos\\(2) pib_paises.csv",
                       sep = ",",
                       dec = ".")
...
# Cotação do dólar comercial (venda e compra) - cotaçõees diárias
...
# Os dados contém as notas de 100 alunos em 4 disciplinas distintas
...
save(banco_dados_um, file = "C:\\Users\\natha\\Downloads\\Introducao ao R e Graficoszip Portugues\\Introdução ao R e Gráficos\\(3) dados_salvos_1.RData")
...
write_xlsx(banco_dados_dois,"C:\\Users\\natha\\Downloads\\Introducao ao R e Graficoszip Portugues\\Introdução ao R e Gráficos\\(3) dados_salvos_2.xlsx")

# Salvando em .csv

write.csv(banco_dados_tres, file = "C:\\Users\\natha\\Downloads\\Introducao ao R e Graficoszip Portugues\\Introdução ao R e Gráficos\\(3) dados_salvos_3.csv", row.names = F)

#------------------- Conceitos básicos de manipulação de dados -----------------

# A manipulação dos dados consiste em organizar as variáveis e observaçõees
...
# Ao todo, quantas variáveis e quantas observaçõees existem no banco de dados?

nrow(desempenho_aluno_escola) # observaçõees (linhas)
...
# Em algumas funçõees, certas vezes, pode ser necessário especificar uma variável
...
# É possível localizar informaçõees do banco de dados da seguinte forma:
...
# Quais são as observaçõees para a variável da 6ª coluna (tipo de escola?)?
...
# As variáveis "horas" e "tempo de experiéncia" poderiam ser excluídas
...
# Também é possível filtrar observaçõees por meio dos operadores:
...
#------------------- Funçõees e iteraçõees ---------------------------------------

# Referéncia: R for Data Science - (https://r4ds.had.co.nz/functions.html)

# Uma função é uma forma de simplificar um código
# É adequada sempre que for necessário repetir o mesmo código várias vezes
# Então, funçõees automatizam tarefas que seriam repetitivas
# Permite que sejam criados e nomeados objetos contendo tais funçõees
...
# Para criar uma função, existem trés etapas básicas:
# 1. Nomear a função
...
# Neste contexto de funçõees, as condiçõees "if, else if e else" são importantes
# Primeiramente, estabelecemos a condição entre parénteses
# Entre as primeiras chaves, vamos estabelecer o resultado caso if = TRUE
# Na sequéncia, estabelecemos o else, ou seja, o restante caso if = FALSE
...
# É possível integrar condiçõees ("if") Ã s funçõees apresentadas anteriormente
...
# Na sequéncia, o else {o que retornar quando NÃƒO for satisfeita}
...
# Do mesmo modo, é possível adicionar múltiplas condiçõees Ã s funçõees ("else if")
...
# Nos exemplos acima, criamos nossas funçõees que utilizamos em cada código 
# Porém, também poderíamos utilizar funçõees já existentes
# Ou seja, o código pode conter uma grande diversidade de funçõees já existentes
...
# Portanto, funçõees e iteraçõees são ferramentas que podem facilitar a escrita
...
library(ggplot2) # Obs.: se já tiver carregado o tidyverse, não precisa fazé-lo

# Os códigos para a criação dos mais diversos tipos de gráficos são semelhantes
# Em esséncia, muda-se a "geometria" do gráfico -> geometria = tipo de gráfico
# Muitos dos argumentos adicionais são ajustes e formataçõees
...
load("C:\\Users\\natha\\Downloads\\Introducao ao R e Graficoszip Portugues\\Introdução ao R e Gráficos\\(2) perfil_investidor.RData")
...
# Conforme indicado, muitos dos argumentos adicionais são ajustes e formataçõees
...
# Vamos adicionar bordas Ã s barras (argumento "color")
...
# Vamos adicionar algumas formataçõees
...
# Na sequéncia, vamos elaborar um gráfico de dispersão dos pontos
# Os dados são do atlas ambiental sobre distritos da cidade de São Paulo

load("C:\\Users\\natha\\Downloads\\Introducao ao R e Graficoszip Portugues\\Introdução ao R e Gráficos\\(2) atlas_ambiental.RData")
...
# Neste caso, vamos utilizar o banco de dados com preços de açõees

library(readxl)
preco_acao <- read_excel("C:\\Users\\natha\\Downloads\\Introducao ao R e Graficoszip Portugues\\Introdução ao R e Gráficos\\(2) precos_acao.xlsx")

# Como temos 4 açõees no banco de dados, vamos implementar o seguinte gráfico
...
ggplot(preco_acao) +
  geom_line(aes(x = data, y = preco, color = acao)) + 
  labs(x = "Més de Referéncia",
       y = "Cotação de Fechamento",
       title = "Série Histórica das Açõees",
       color = "Empresa") +
  theme_classic()
...
# Basta indicar com o cursor do mouse o ponto para ver informaçõees
...
# Vamos gerar um gráfico de calor que distingue informaçõees por meio de cores
# O banco de dados contém informaçõees sobre o tempo para chegar Ã  escola
# Fonte: Fávero & Belfiore (2017, Capítulo 12)

library(readxl) # Se já tiver carregado antes, não precisa fazer novamente
tempo_dist <- read_excel("C:\\Users\\natha\\Downloads\\Introducao ao R e Graficoszip Portugues\\Introdução ao R e Gráficos\\(2) tempo_dist.xls")

# Vamos trabalhar o gráfico de calor no contexto das correlaçõees entre variáveis
# Portanto, primeiramente, vamos criar a matriz de correlaçõees (função "cor")
...
# Neste caso, vamos colocar os valores das correlaçõees como uma variável
...
# Agora vamos elaborar um gráfico de calor (geom_tile) com algumas formataçõees
